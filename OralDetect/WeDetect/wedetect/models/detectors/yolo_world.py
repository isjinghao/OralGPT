...
class YOLOWorldDetector(YOLODetector):
    ...
    def __init__(self,
        ...
        # Optional modality-conditioned semantic calibration of the class prototypes.
        # None -> the model is byte-for-byte the stock detector. See layers/modality_calib.py.
        self.modality_calib = MODELS.build(modality_calib) if modality_calib is not None else None
    ...
    def extract_feat(
        ...
        # Modality-conditioned calibration of the class prototypes. Applied on the BACKBONE feature
        # maps (before the neck), because the deepest backbone map is what carries modality identity.
        # Runs on the reparameterised path too: the cached BERT prototypes still get calibrated
        # against the image actually being scored.
        if getattr(self, 'modality_calib', None) is not None:
            txt_feats = self.modality_calib(img_feats, txt_feats)
