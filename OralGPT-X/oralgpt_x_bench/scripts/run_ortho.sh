#!/usr/bin/env bash
# Ortho benchmark: infer -> pixel metrics -> summarize
set -euo pipefail
...
if [[ "${FULL_EVAL:-0}" == "1" ]]; then
  ...
  bench_data_root="${bench_data_root:-${BENCH_ROOT}/benchmark_data/ortho}"
  PARQUET_ROOT="${parquet_root:-/data/OralGPT/OralGPT-X/dataset_OrthoSurgery/test}"
  if [[ ! -f "${META}" ]]; then
    echo "Exporting full Ortho test split from ${PARQUET_ROOT}..."
    python "${BENCH_ROOT}/tools/export_ortho_benchmark.py" \
...
echo "Ortho evaluation complete. Summary: ${SUMMARY_DIR}/${SUBSET}_summary.json"
