#!/usr/bin/env bash
# CBCT benchmark: infer -> pixel metrics -> summarize
set -euo pipefail
...
if [[ "${FULL_EVAL:-0}" == "1" ]]; then
  ...
  bench_data_root="${bench_data_root:-${BENCH_ROOT}/benchmark_data/cbct}"
  PARQUET_ROOT="${parquet_root:-/data/OralGPT/OralGPT-X/dataset_CBCT_Low-Dose_to_Standard/test}"
  if [[ ! -f "${META}" ]]; then
    echo "Exporting full CBCT test split from ${PARQUET_ROOT}..."
    python "${BENCH_ROOT}/tools/export_cbct_benchmark.py" \
...
echo "CBCT evaluation complete. Summary: ${SUMMARY_DIR}/${SUBSET}_summary.json"
