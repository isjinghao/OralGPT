...
DISEASE_MATCHERS = {
    "caries": [r"\bcaries\b", r"\bcarious lesion\b", r"\bcavity\b", r"\bcavities\b", r"\bdecay\b"],
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Rewrite dental panoramic X-ray reasoning dialogue data using a GPT API",
    ...
    parser.add_argument(
        "--input-path",
        type=str,
        default="/hpc2hdd/home/yfan546/workplace/mllm_playground/mllm_2/r1/mmoral_reasoning_data/multi_round_output_data3/multi_round_data3.json",
        help="Input JSON file path"
    )
    parser.add_argument(
        "--output-path",
        type=str,
        default="/hpc2hdd/home/yfan546/workplace/mllm_playground/mllm_2/r1/mmoral_reasoning_data/multi_round_output_data3/multi_round_data3_step_2.json",
        help="Output JSON file path"
    )
    parser.add_argument(
        "--checkpoint-path",
        type=str,
        default="/hpc2hdd/home/yfan546/workplace/mllm_playground/mllm_2/r1/mmoral_reasoning_data/multi_round_output_data3/checkpoint.json",
...
def make_assistant_turn_messages(assistant_content: str,
    ...
    """
    Build the system + user messages that ask the model to rephrase a single
    assistant turn. If gpt-4-analysis is provided, its key insights should be
    woven naturally into the rewritten <think> (and <answer>), not pasted in.
    """
    sys_text = (
...
def call_gpt_messages(messages: List[Dict[str, Any]],
    ...
    """Call the API and force a JSON-formatted response."""
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            try:
                ...
                raise ValueError(f"Response content is not in the expected JSON format: {parse_err}")
...
def parse_turn_json(raw: str, contains_answer: bool) -> Dict[str, str]:
    ...
    if contains_answer:
        ...
        if not re.search(r'<\s*answer\s*>', answer_content, flags=re.I):
            raise ValueError("'answer' field must contain an opening <answer> tag")
        if not re.search(r'</\s*answer\s*>', answer_content, flags=re.I):
            raise ValueError("'answer' field must contain a closing </answer> tag")
...
def rewrite_assistant_message_content(content: str, gpt4_analysis: Optional[str], args) -> str:
    """
    Rewrite only the <think> (and <answer> if present) of the current assistant
    content. The <tool_call> block is kept verbatim and reattached here.
    """
...
def process_single_sample(sample: Dict[str, Any], idx: int, args) -> Dict[str, Any]:
    """
    Iterate over sample["messages"] and rewrite the <think> (and <answer> if
    present) of each assistant message; other fields are kept unchanged and the
    <tool_call> block is preserved verbatim. If an assistant message fails to be
    rewritten, raise an exception to skip the whole sample.
    """
    ...
    for mi, m in enumerate(messages):
        ...
        last_err = None
        rewritten = None
        for msg_attempt in range(1, args.max_sample_retries + 1):
            try:
                if msg_attempt > 1:
                    print(f"    Message {mi} retry {msg_attempt}/{args.max_sample_retries}")
                else:
                ...
                last_err = e
                print(f"    Message {mi} rewrite failed (attempt {msg_attempt}/{args.max_sample_retries}): {e}")
                if msg_attempt < args.max_sample_retries:
                    backoff = min(2 ** msg_attempt, 20)
                    print(f"    Waiting {backoff}s before retrying message...")
                    time.sleep(backoff)

        if rewritten is None:
            raise RuntimeError(f"Message {mi} still failed after {args.max_sample_retries} retries: {last_err}")
...
def process_all(args):
    ...
    for count, idx in enumerate(pending, 1):
        ...
        try:
            ...
            print(f"  Sample #{idx} ultimately failed, skipped: {e}")
