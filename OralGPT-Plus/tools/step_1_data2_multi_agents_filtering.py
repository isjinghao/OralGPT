    ...
    for proposal_idx, proposal in enumerate(all_proposals):
        ...
        if DEBUG_LOG_RESPONSES:
            if crop_resp.get("success"):
                tqdm.write(f"[ROUND {round_number}][CROP] bbox={bbox}")
                trunc_raw = crop_resp["gpt_answer"] if len(crop_resp["gpt_answer"]) <= 1000 else crop_resp["gpt_answer"][:1000] + "...[truncated]"
                tqdm.write(f"[ROUND {round_number}][CROP][RAW] {trunc_raw}")
                trunc_ans = crop_answer if len(crop_answer) <= 600 else crop_answer[:600] + "...[truncated]"
                tqdm.write(f"[ROUND {round_number}][CROP][Answer] {trunc_ans}")
            else:
                tqdm.write(f"[ROUND {round_number}][CROP][FAIL] {crop_resp.get('error')}")
        ...
        if DEBUG_LOG_RESPONSES:
            if comp_resp.get("success"):
                tqdm.write(f"[ROUND {round_number}][COMP] bbox={bbox} mirror_bbox={mirror_bbox}")
                trunc_raw = comp_resp["gpt_answer"] if len(comp_resp["gpt_answer"]) <= 1000 else comp_resp["gpt_answer"][:1000] + "...[truncated]"
                tqdm.write(f"[ROUND {round_number}][COMP][RAW] {trunc_raw}")
                trunc_ans = comp_answer if len(comp_answer) <= 600 else comp_answer[:600] + "...[truncated]"
                tqdm.write(f"[ROUND {round_number}][COMP][Answer] {trunc_ans}")
            else:
                tqdm.write(f"[ROUND {round_number}][COMP][FAIL] {comp_resp.get('error')}")

        guide_resp = get_llm_text_response("ground_truth_guidance", gt_description)
        guided_answer = extract_answer_from_response(guide_resp["response"]) if guide_resp["success"] else gt_description
        if DEBUG_LOG_RESPONSES:
            if guide_resp.get("success"):
                trunc_raw = guide_resp["response"] if len(guide_resp["response"]) <= 1000 else guide_resp["response"][:1000] + "...[truncated]"
                tqdm.write(f"[ROUND {round_number}][GUIDE][RAW] {trunc_raw}")
                trunc_ans = guided_answer if len(guided_answer) <= 600 else guided_answer[:600] + "...[truncated]"
                tqdm.write(f"[ROUND {round_number}][GUIDE][Answer] {trunc_ans}")
            else:
                tqdm.write(f"[ROUND {round_number}][GUIDE][FAIL] {guide_resp.get('error')}")
...
def main():
    ...
    parser.add_argument('--json_path', type=str,
                        default="/hpc2hdd/home/yfan546/workplace/mllm_playground/mllm_2/r1/source_data/2_400/Multimodal_data_Sichuan_Uni_Children.json",
                        help='JSON file path')
    parser.add_argument('--proposals_path', type=str,
                        default="/hpc2hdd/home/yfan546/workplace/mllm_playground/mllm_2/r1/source_data/2_400/proposals_data2.json",
                        help='Proposals JSON file path')
    parser.add_argument('--data_path', type=str,
                        default="/hpc2hdd/home/yfan546/workplace/mllm_playground/mllm_2/r1/source_data/2_400/",
                        help='Data root directory path')
    parser.add_argument('--output_path', type=str,
                        default='/hpc2hdd/home/yfan546/workplace/mllm_playground/mllm_2/r1/mmoral_reasoning_data/multi_round_output_data2',
