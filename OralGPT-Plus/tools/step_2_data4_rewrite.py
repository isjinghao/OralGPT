...
def parse_args():
    ...
    parser.add_argument(
        "--input-path",
        type=str,
        default="/hpc2hdd/home/yfan546/workplace/mllm_playground/mllm_2/r1/mmoral_reasoning_data/multi_round_output_data4/multi_round_data.json",
        help="Input JSON file path"
    )
    parser.add_argument(
        "--output-path",
        type=str,
        default="/hpc2hdd/home/yfan546/workplace/mllm_playground/mllm_2/r1/mmoral_reasoning_data/multi_round_output_data4/multi_round_data4_step_2.json",
        help="Output JSON file path"
    )
    parser.add_argument(
        "--checkpoint-path",
        type=str,
        default="/hpc2hdd/home/yfan546/workplace/mllm_playground/mllm_2/r1/mmoral_reasoning_data/multi_round_output_data4/checkpoint.json",
...
def get_response_schema(contains_answer: bool) -> Dict[str, Any]:
    """Build a strict JSON Schema to force the model output into the required format."""
    if contains_answer:
        return {
            "type": "object",
            "properties": {
...
def process_single_sample(sample: Dict[str, Any], idx: int, args) -> Dict[str, Any]:
    ...
    <tool_call> block is preserved verbatim.
    """
    ...
    for count, idx in enumerate(pending, 1):
        ...
        for attempt in range(1, args.max_sample_retries + 1):
            try:
                ...
                done_set.add(idx)
                if attempt > 1:
                    print(f"  Sample #{idx} retry succeeded (attempt {attempt})")
                else:
