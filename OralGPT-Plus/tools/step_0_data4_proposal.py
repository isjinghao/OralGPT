...
# -*- coding: utf-8 -*-
"""
Enhanced: add optional visualization and sample count limiting.

- Visualization (--visualize):
  Draws Precise Grounding Position (target categories) and final proposals on
  the original image (found via --image-root + basename(image_name)).
  Saves to --vis-out directory.

- Sample limiting:
  --max-samples N : process at most N samples (after offset)
  --sample-offset M : skip the first M samples

Other core logic is identical to the previous version.

[UPDATED]
- For ALL 'other' categories: match to multiple teeth via IoU>0 using contextual boxes.
- For 'bone resorbtion/loss': same multi-tooth IoU>0 matching using contextual.
- If a lesion matches NO tooth: enlarge its contextual bbox by +20% (w,h) for downstream proposal computation.
"""
...
def pad_box(b: List[int], img_w: int, img_h: int, pad_ratio: float=0.03) -> List[int]:
    pad_x = int(img_w * pad_ratio)
    pad_y = int(img_h * pad_ratio)
    return [max(0, b[0]-pad_x), max(0, b[1]-pad_y), min(img_w, b[2]+pad_x), min(img_h, b[3]+pad_y)]

# UPDATED: enlarge a bbox by +pct (e.g., +20%) relative to its width/height
...
BONE_KEYS = {'bone resorbtion', 'bone loss'}
OTHER_SET = {
    'carious lesion',
...
def is_bone_resorbtion(cat: str) -> bool:
    c = normalize_category(cat)
    return any(k in c for k in BONE_KEYS)
...
def choose_k_for_bone(n: int) -> int:
    if n <= 2: return 1
    if n <= 4: return 2
    if n <= 7: return 3
    if n <= 12: return 4
...
# -----------------------
# Proposal merging for 'other' type (existing)
# -----------------------
...
# NEW: generic merger for IoM70 (applies to each type separately)
...
COLOR_PRECISE = {
    'bone': (255, 0, 0),
    'carious lesion': (255, 165, 0),
    'apical periodontitis': (0, 200, 0),
    'furcation lesion': (0, 128, 255),
    'root resorption': (255, 0, 255),
    'root fragment': (128, 0, 128),
    'endodontic treatment': (128, 128, 128),
    # 'impacted': (200, 0, 100),
    'other': (255, 255, 255),
}
COLOR_PROPOSAL = {
    'bone_resorbtion': (255, 0, 0),
    'other': (255, 255, 0),
}
...
def categorize_for_precise(cat: str) -> str:
    if is_bone_resorbtion(cat): return 'bone'
    c = normalize_category(cat)
...
def visualize_sample(image_root: str,
    ...
    for i, b in enumerate(precise):
        ...
        if is_bone_resorbtion(cats[i]) or is_other_target(cats[i]):
...
# -----------------------
# Main per-image logic (UPDATED matching for bone + others)
# -----------------------
def process_image_sample(sample: Dict[str, Any],
                         teeth_dir: str,
                         area_thresh_bone: int = 2415,  # kept for compatibility; not used in new matching
    ...
    full_answer_text = sample.get('Full Answer', '') or sample.get('Full answer', '') or ''
    ...
    for i, cat in enumerate(categories):
        if i >= len(precise):
            continue
        if is_bone_resorbtion(cat):
            bone_idx.append(i)
        elif is_other_target(cat):
            other_idx.append(i)

    proposals: List[Dict[str, Any]] = []

    # ---------- BONE (multi-tooth IoU>0 via contextual; enlarge 20% if no match) ----------
    if len(bone_idx) > 0:
        bone_pts = []
        bone_items = []  # (cx,cy), lesion_index, matched_teeth_indices(list), used_box_for_union
        for i_lesion in bone_idx:
            ...
            # If no matched tooth => enlarge contextual by +20% for downstream union
            ...
            bone_items.append(((cx, cy), i_lesion, matched_teeth, used_box))

        if bone_items:
            pts = np.array(bone_pts, dtype=np.float64)
            K = choose_k_for_bone(len(bone_items))
            labels = np.zeros(len(bone_items), dtype=np.int32) if K <= 1 else kmeans(pts, min(K, len(bone_items)))

            for cl in range(labels.max()+1):
                lesion_indices = [bone_items[k][1] for k in range(len(bone_items)) if labels[k] == cl]
                # unify matched teeth across lesions in this cluster
                cluster_teeth_idx: Set[int] = set()
                cluster_boxes: List[List[int]] = []
                for k in range(len(bone_items)):
                    if labels[k] != cl:
                        continue
                    _, _, matched_teeth, used_box = bone_items[k]
                ...
                if teeth_struct is not None and len(cluster_teeth_idx) > 0:
                    ...
                    # no matched tooth in this cluster -> only union enlarged context boxes
                    bbox = box_union(cluster_boxes) if cluster_boxes else [0, 0, img_w, img_h]

                proposals.append({
                    "type": "bone_resorbtion",
                    "cluster_id": int(cl),
...
def main():
    ...
    parser.add_argument("--teeth-json-dir", type=str, default='/hpc2hdd/home/yfan546/workplace/mllm_playground/mllm_2/r1/source_data/4_1600/id_jsons',
                        help="Directory of per-image teeth jsons (e.g., .../id_jsons)")
    parser.add_argument("--new-json", type=str, default='/hpc2hdd/home/yfan546/workplace/mllm_playground/mllm_2/r1/source_data/4_1600/processed_annotations.json',
                        help="Path to the aggregated new_Multimodal_data JSON")
    parser.add_argument("--out", type=str, default='./proposals_data4.json',
                        help="Output path for proposals JSON")
    parser.add_argument("--area-thresh-bone", type=int, default=2415,
                        help="(Kept for compatibility) Area threshold for bone resorbtion when count <= 2 (not used in updated matching)")
    ...
    parser.add_argument("--image-root", type=str,
                        default="/hpc2hdd/home/yfan546/workplace/mllm_playground/mllm_2/r1/source_data/4_1600/all/images",
    ...
    for idx in range(start, end):
        ...
        try:
            out = process_image_sample(sample, args.teeth_json_dir, args.area_thresh_bone, logger)
