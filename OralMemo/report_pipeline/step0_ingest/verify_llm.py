...
def verify_timeline(client, raw_dir: Path, timeline: dict, captions: list[dict] | None = None) -> dict:
...
def high_severity_issues(verification: dict) -> list[dict]:
    return [i for i in verification.get("issues", []) if i.get("severity") == "high"]
