...
def _score_and_checkpoint(
    ...
    write_json_atomic(report_path, method_report)
...
def evaluate_trajectory(
    ...
    *,
    ...
    def answer_method(method):
        ...
        if not answers_rebuilt:
            ...
            try:
                ...
                write_json_atomic(answers_path, records)
            except Exception:
                if not setup_succeeded:
                    method.add_metrics(failures=1)
                raise
            finally:
                write_json_atomic(metrics_path, method.metrics())
                try:
                    method.close()
                finally:
    ...
    report = {"methods": [method_reports[name] for name in methods]}
...
def aggregate_trajectory(out: Path, trajectory_name: str, methods: list[str], answer_model: str) -> dict:
    trajectory = read_json(resolve_trajectory_path(out, trajectory_name, answer_model))
    eval_root, _ = evaluation_roots(out, trajectory["trajectory_type"], answer_model)
    reports = []
    for method in methods:
        report_path = eval_root / method / "report.json"
        report = read_json(report_path)
        if report.get("failed_tasks"):
            raise RuntimeError(f"Cannot aggregate incomplete scoring report: {report_path}")
    ...
    write_json_atomic(eval_root / "report.json", report)
    csv_path = eval_root / "report.csv"
    tmp_path = csv_path.with_suffix(".csv.tmp")
    tmp_path.write_text(format_csv(report), encoding="utf-8")
...
def trajectory_scoring_completed(
    ...
    for method in methods:
        ...
        report_path = method_root / "report.json"
        if not (method_root / "answers.json").is_file() or not report_path.is_file():
            return False
        try:
            report = read_json(report_path)
        except ValueError:
            return False
        if report.get("failed_tasks") or "memory_metrics" not in report:
            return False
    return True
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--aggregate-only", action="store_true", help="Build aggregate reports from completed method reports")
...
def main() -> int:
    ...
    if args.aggregate_only and args.phase != "scoring":
        raise ValueError("--aggregate-only requires --phase scoring")
    ...
    def worker(item: dict) -> str:
        ...
        if args.aggregate_only:
            for trajectory_name in args.trajectories:
                aggregate_trajectory(out, trajectory_name, args.methods, answer_model)
            return "completed"
