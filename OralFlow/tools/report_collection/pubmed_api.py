...
DB = "pubmed"
NCBI_FETCH_MAX_ATTEMPTS = max(1, int(os.getenv("NCBI_FETCH_MAX_ATTEMPTS", "4")))
NCBI_FETCH_RETRY_BASE_DELAY = float(os.getenv("NCBI_FETCH_RETRY_BASE_DELAY", "1.5"))
NCBI_FETCH_MAX_DELAY = float(os.getenv("NCBI_FETCH_MAX_DELAY", "30"))
NCBI_DEFAULT_TOOL = "OralFlow"
...
def _retry_delay(attempt: int) -> float:
    return min(NCBI_FETCH_MAX_DELAY, NCBI_FETCH_RETRY_BASE_DELAY * (2 ** (attempt - 1)))
...
def _ncbi_get_with_retry(url: str, *, params: Dict[str, Any], timeout: float | int) -> requests.Response:
    ...
    for attempt in range(1, NCBI_FETCH_MAX_ATTEMPTS + 1):
        try:
            _throttle_ncbi()
            ...
            if resp.status_code in NCBI_RETRY_STATUS_CODES:
                if attempt >= NCBI_FETCH_MAX_ATTEMPTS:
                    resp.raise_for_status()
                sleep_s = _retry_delay(attempt)
                time.sleep(sleep_s)
                continue
            resp.raise_for_status()
            return resp
            ...
            if attempt >= NCBI_FETCH_MAX_ATTEMPTS:
                break
            sleep_s = _retry_delay(attempt)
            print(f"NCBI GET retry {attempt}/{NCBI_FETCH_MAX_ATTEMPTS} after transient error: {e}", file=sys.stderr)
            time.sleep(sleep_s)
    ...
    for attempt in range(1, NCBI_FETCH_MAX_ATTEMPTS + 1):
        try:
            ...
            if attempt >= NCBI_FETCH_MAX_ATTEMPTS:
                break
            sleep_s = _retry_delay(attempt)
            print(f"NCBI POST retry {attempt}/{NCBI_FETCH_MAX_ATTEMPTS} after transient error: {e}", file=sys.stderr)
            time.sleep(sleep_s)
    if last_err is not None:
        raise last_err
