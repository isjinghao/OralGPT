...
```bash
cd /root/autodl-tmp/OralGPT/OralFlow
...
docker run -d \
  ...
  -v oralflow_neo4j_data:/data \
  neo4j:5.26.8

# 完整实验前确认服务已运行
docker start oralflow-neo4j
docker ps --filter name=oralflow-neo4j
docker logs -f oralflow-neo4j
```

连接检查：

```bash
docker exec oralflow-neo4j cypher-shell \
...
```bash
docker rm -f oralflow-neo4j
docker volume rm oralflow_neo4j_data
```
